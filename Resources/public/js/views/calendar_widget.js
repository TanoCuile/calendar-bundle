function initializeCalendarWidgetView(base) {
    base.CalendarView = Backbone.View.extend({
        ...
        popupTemplate: '<div class="note-data-edit">' +
            '<div class="close glyphicon glyphicon-remove" rv-on-click="view.closePopup"></div>' +
            '<div><span rv-value="note:hours"></span>:<span rv-value="note:minutes"></span></div>' +
            '<form class="">' +
            '<div class="form-group">' +
            '<label for="phone">Телефон:</label><input type="text" rv-value="note:phone" id="phone" /></div>' +
...
//                }.bind(this));
        ...
        // set new master list
        setMasters: function(masters, ignoreActive) {
            var show = false;
        ...
        setNote: function(e, options){
            ...
            var master = this.options.masters[0];
            var job = base.serviceWidget.price.get('jobType');
            var note = null;
            if (job) {
                note = new base.OrderNote({
                    'date': base.calendar.date,
                    'fromTime': time,
                    'toTime': time + parseInt(base.serviceWidget.duration.get('duration')),
                    'service': job,
                    'editable': true,
                    'owner': master,
                    'new': true,
                ...
                //////console.log(time + parseInt(base.serviceWidget.duration.get('duration')));
...
//                });
                if(note && note.validate(
                    base.CalendarView.getStartTime(note, this.options),
                    base.CalendarView.getEndTime(note, this.options),
                    this.options.notes)
                    ) {
                    //hideBSMessage();
                    this.calendar.hideMessage();
                    this.calendar.widget.options.notes[note.get('id')] = note;
                    this.calendar.widget.renderNotes();
                    this.calendar.widget.options.activeNote = note.get('id');
                }
            }
        },
        // show controls popup
        ...
        closePopup: function(e, parameters) {
            parameters.view.$el.find('.table').removeClass('hidden');
            parameters.view.clearPopup();
            delete parameters.view.options.notes[parameters.view.options.activeNote];
            parameters.view.calendar.calendarShowHeader();
            ////console.log(parameters.view.options.notes);
            parameters.view.renderNotes();
        },
        clearPopup: function() {
            if (this.popup){
                this.popup.unbind();
        ...
        saveNote: function(e, parameters) {
            ...
            if (parameters.note.validate(
                base.CalendarView.getStartTime(parameters.note, parameters.view.options),
                base.CalendarView.getEndTime(parameters.note, parameters.view.options),
        ...
        renderNote: function(note, draggable) {
            ...
            for (var i = first; i < toTime; i += 15) {
                var td = this.$el.find('tr[data-time="' + i + '"] td.cell');
                ...
                if (draggable || view.draggable == false) {
                    ...
                    td.bind('click', function(e) {
                        var note = $(e.target).attr('data-note');
                        if (note && view.options.notes[note].get('editable')) {
                            e.preventDefault(true);
        ...
        drag: function(e, ui){
            ...
            if (e.which === 1 && leftButtonDown && base.cDrag) {
                base.cDrag.move = true;
                ...
                if (Math.abs(y) >= view.options.height || Math.abs(x) > 60) {
                    ...
                    if (change) {
                        if (noteObj.validate(
                            base.CalendarView.getStartTime(noteObj, view.options),
                            base.CalendarView.getEndTime(noteObj, view.options),
                            view.options.notes)
                            ) {
        ...
        getStartTime: function(note, options) {
            var dayInfo = false;
            if (dayInfo = base.SingleCalendarView.getMasterCurrentDayInfo(note.get('owner'))) {
                return dayInfo.get('fromTime');
            }
    ...
    $(document).mouseup(function(e){
        if (base.cDrag && base.cDrag.move) {
            ...
            if (note.validate(
                base.CalendarView.getStartTime(note, options),
                base.CalendarView.getEndTime(note, options),
                options.notes)
                ) {
